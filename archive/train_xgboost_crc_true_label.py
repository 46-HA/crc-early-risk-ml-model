...
def main():
    ...
    ap.add_argument("--table", default="output/crc_patient_table_m7_12.csv")
    ...
    for t in [thr, 0.92, 0.94, 0.96, 0.98]:
    ...
    for t in [0.85, 0.80, 0.75]:
        tp, fp, fn, tn, prec, rec, fpr = confusion_at_threshold(y_te, p_te, t)
        print("TEST thr=%.2f prec=%.3f rec=%.3f fpr=%.3f tp=%d fp=%d fn=%d tn=%d" % (t, prec, rec, fpr, tp, fp, fn, tn))
